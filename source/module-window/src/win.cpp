...
namespace xwn {
	...
	void setMouseButton(xen::WindowEvent& e, xen::WindowEvent::Type type, xen::MouseButtons::Values button, WPARAM wParam, LPARAM lParam) {
		e.type = type;
	...
	xen::Key xenKeyFromVirtualKey(WPARAM wParam, LPARAM lParam) {
		if (wParam >= 'A' && wParam <= 'Z') {
			return static_cast<xen::Key>((u32)xen::Key::A + (wParam - 'A'));
		}
		else if (wParam >= '0' && wParam <= '9') {
			return static_cast<xen::Key>((u32)xen::Key::Num0 + (wParam - '0'));
		}
		else if (wParam >= VK_F1 && wParam <= VK_F24) {
			return static_cast<xen::Key>((u32)xen::Key::F1 + (wParam - VK_F1));
		}
		else if (wParam >= VK_NUMPAD0 && wParam <= VK_NUMPAD9) {
			return static_cast<xen::Key>((u32)xen::Key::Numpad0 + (wParam - VK_NUMPAD0));
		}
		else {
			switch (wParam) {
	...
	UINT virtualKeyFromXenKey(xen::Key key) {
		u32 value = (u32)key;
		if (value >= (u32)xen::Key::A && value <= (u32)xen::Key::Z) {
			return 'A' + (value - (u32)xen::Key::A);
		}
		else if (value >= (u32)xen::Key::Num0 && value <= (u32)xen::Key::Num9) {
			return '0' + (value - (u32)xen::Key::Num0);
		}
		else if (value >= (u32)xen::Key::F1 && value <= (u32)xen::Key::F24) {
			return VK_F1 + (value - (u32)xen::Key::F1);
		}
		else if (value >= (u32)xen::Key::Numpad0 && value <= (u32)xen::Key::Numpad9) {
			return VK_NUMPAD0 + (value - (u32)xen::Key::Numpad0);
		}
		else {
			switch (key) {
	...
	void startMouseTracking(xen::Window* window) {
	...
	LRESULT CALLBACK windowEventCallback(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
		...
		if (w == NULL) {
			...
			case WM_KEYDOWN:
				...
				e.key = xenKeyFromVirtualKey(wParam, lParam);
...
void* init(const void* params){
	...
	wc.lpfnWndProc = xwn::windowEventCallback;
	wc.hInstance = module;
